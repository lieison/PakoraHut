...
------------------------------------ */
	...
	$preview_skin = ( get_option('skin_select') !='' ) ? get_option('skin_select') : '';
...
<body <?php body_class('skinset-background nv-skin '. $preloader ); ?>>
	...
	{	
		...
		echo "\n\t". '<div id="custom-layer2" class="custom-layer '. $NV_show_slider .' '. $autohide .'">';		
	...
	echo "\n\t". '<div id="primary-wrapper" class="'. ( !empty( $fullscreen_slider ) ? 'fullslider-active' : ''  ) .' '. ( $NV_wide_layout == 'enable' ? 'tva-wide-layout' : ''  ) .' '. $collapse_menu .'">';
	echo "\n\t\t". '<a id="top"></a>';
...
------------------------------------ */
	...
	if(  ! post_password_required() && ( !empty( $NV_show_slider ) && $fullscreen_slider != 'yes' ) )
	{
