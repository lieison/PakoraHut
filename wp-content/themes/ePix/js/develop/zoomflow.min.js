...
(function($) {
	...
	var tvaCarouselAjaxLoadData = function( container, cthis, currNr )
	{
		var type			= $( container ).attr('data-type'), 
			...
			data_offset 	= $( container + ' .dynamic-frame').attr('data-offset'),	
		...
		$.ajax({
		  ...
		  {
				$( container + ' .dynamic-frame').attr('data-offset', parseInt( data_offset ) + parseInt( load_value ) );	
									
				if( data != '' )
				{
					jQuery( container + ' .items' ).append( jQuery( data ) );
					jQuery( container + ' .the-items' ).remove();		
					jQuery( container + ' .the-structure-holder' ).remove();
					jQuery( container + ' .preloader' ).remove();
		
					var slider = container,
						timeout = $( slider ).attr("data-timeout"),
						ratio = $( slider ).attr("data-ratio"),
						padding = 0,
						maxwidth = $( slider ).attr("data-max-width");
						
					$( slider ).zoomflow({
						settings_slideshowTime: parseInt( timeout ),
						settings_slideshow:"on",
						design_padding: padding,
						design_ratio: ratio,
						tempNr: currNr,
						design_maxwidth: maxwidth,
						design_enableShadows: 'off' // enable canvas shadows
					});
				}
    ...
    return this.each(function() {
        ...
        function init(){
           if(cthis.attr('class').indexOf("skin-")==-1){
            ...
            nrChildren = cthis.find('.items-wrap').children('.items').eq(0).children().length;
        ...
        function setupStructure(){
            _items = cthis.find('.items-wrap').children('.items');
            //var elementToAppend = $('<div />');
            cthis.find('.items-wrap').append('<div class="preloader"></div>');
            cthis.find('.items-wrap').append('<div class="the-structure-holder"></div>');
            cthis.find('.items-wrap').append('<div class="the-items"></div>');
            //console.log(cthis);
            _theItems = cthis.find('.items-wrap').children('.the-items');
            ...
            for(i=0; i<_items.children().length; i++){
                ...
                if(the_type=='image'){
					...
					{
						_theItems.children().eq(i).append('<div class="the-bg" style="background-image: url('+_c.attr('data-source')+')"></div>');
					}
                }
                ...
                _theItems.children().eq(i).append('<div class="controlsCon"><div class="arrow-left"><i class="fa fa-chevron-left fa-lg"></i></div><div class="arrow-right"><i class="fa fa-chevron-right fa-lg"></i></div></div>');
            }
        ...
        function handle_loaded(){
            ...
            _theItems.children().on('click', click_item);
        ...
        if(allowed==false){
			...
			otherItemMarginLeft = -(ltw*ratio)/2;
			
			var prevItemMargin = -(ltw*ratio)/2;
        }
        ...
        function gotoItem(arg){
            ...
            if(currNr>-1){
				...
				{				
					currItem.animate({
						'margin-left' : currItemMarginLeft,
						'left' : '50%',
						'padding' : '0'
					}, 700, 'easeOutSine' );
				}
                ...
                if(mode!="only-two")
				{
					...
					{
						prevItem.animate({
							'margin-left' : otherItemMarginLeft,
							'left' : '27%',
							'padding' : '3% 0'
						}, 800, 'easeOutSine' );
					}
            ...
            currNr=arg;
			 ...
			 var slide_id = '#'+ cthis.attr('id'),
			 	 slides_total = $( slide_id + ' .dynamic-frame').attr('data-total'),
				 items = $( slide_id ).find('.the-items').children();


			 if( $( slide_id ).attr('data-load-method') == 'auto_load' && items.last().hasClass('currItem') && items.length < parseInt( slides_total ) )
			 { 
				tvaCarouselAjaxLoadData( slide_id, cthis, items.length-1 );
			 }
...
(function( $ )
{
	...
	{
		$('.carousel.zoomflow').each(function(index, value) { 	
