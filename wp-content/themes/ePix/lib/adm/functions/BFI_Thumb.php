<?php
/*
 * bfi_thumb - WP Image Resizer ( modified by themeva )
 *
...
if( !class_exists( 'BFI_Thumb' ) )
{
	...
	class BFI_Image_Editor_GD extends WP_Image_Editor_GD {
		...
		public function rotate( $angle ) {
			...
			return new WP_Error( 'image_rotate_error', __('Image rotate failed.','themeva_admin'), $this->file );
		}
		...
		public function opacity($opacity) {
			...
			return new WP_Error( 'image_opacity_error', __('Image opacity change failed.','themeva_admin'), $this->file );
		}
		...
		public function colorize($hexColor) {
			...
			return new WP_Error( 'image_colorize_error', __('Image color change failed.','themeva_admin'), $this->file );
		}
		...
		public function grayscale() {
			...
			return new WP_Error( 'image_grayscale_error', __('Image grayscale failed.','themeva_admin'), $this->file );
		}
		...
		public function negate() {
			...
			return new WP_Error( 'image_negate_error', __('Image negate failed.','themeva_admin'), $this->file );
		}
	}
	...
	class BFI_Thumb {
		...
		public static function thumb($url, $params = array(), $single = true) {
			...
			if( empty( $params ) && !empty( $url ) ) return $url;
			...
			if(strpos( $url, $site_url ) === false && strpos( $url, 'http://' ) === false )
			{		
				if( strpos( $site_url . $url, $upload_url ) !== false )
				{
					$url = $site_url . $url;
				}
			}
			...
			if (isset($width)) {
				...
				// Set Max Width if Mobile Device
				global $deviceType;
				...
				if( !empty( $deviceType ) )
				{
					...
					{
						if( $deviceType == 'phone' && $width > 400 )
						{
							$width = 400;
						}
						elseif( $deviceType == 'tablet' && $width > 800 )
						{
							$width = 800;
						}
					}
