 ...
 */
	...
	( function( $ )
	{
		...
		$(document).ready(function()
		{
			...
			function show_options(type, id, value, layer)
			{
				...
				$( '#'+ id +' .'+ type + layer ).each(function() {
					...
					{		
						...
						{
							$( '.data-options'+ layer ).css('display','none');
						}
					}
					...
					if( type == 'layer-options' )
					{
						if( $(layer + '.layer-options-type select').val() == layer.replace(".", "") +'_cycle' )
						{
							$( '#customize-control-'+ $( layer + '.data-source-type select' ).val() ).css('display','block');
						}
					}				
			...
			// Background Types	
			$( '#accordion-section-themeva_frame li,#accordion-section-themeva_header li,#accordion-section-themeva_footer li,#accordion-section-themeva_sidebar li' ).each(function()
			{
