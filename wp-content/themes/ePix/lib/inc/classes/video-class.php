<?php
	...
	if( $NV_videotype == "oembed" || empty( $NV_videotype ) )
	{		
		...
		{
			$output .= do_shortcode('[video src="'. $media_url .'" '. $height .' '. $width .' ]');
		}
		else
		{
			global $wp_embed;
			$output .= $wp_embed->run_shortcode( '[embed '. $height .' '. $width .' ]'. $media_url .'[/embed]' );
		}
	}
	...
	{	
		...
		$output .= '<iframe frameborder="0" '. $height .' '. $width .' marginheight="0" marginwidth="0" src="http://www.youtube.com/embed/'. $vidid . $splitter .'autoplay='. $NV_videoautoplay .'&amp;loop='. $NV_loop . $params .'&amp;wmode=opaque&amp;title=" allowfullscreen></iframe>';
	...
	{ 
    	$output .= '<iframe frameborder="0" marginheight="0" marginwidth="0"  src="http://player.vimeo.com/video/'. $vidid .'?autoplay='. $NV_videoautoplay .'&amp;loop='. $NV_loop .'&amp;title=0&amp;byline=0&amp;portrait=0&amp;" '. $height .' '. $width .' ></iframe>';
	...
	elseif( $NV_videotype == "jwp" )
	{
		...
		$output .= '<div class="jwplayer-wrapper"><div id="'. $slide_id .'" class="jwplayer-container" data-jw-width="'. $NV_vidwidth .'" data-jw-height="'. $jwplayer_height .'" data-jw-media="'. $NV_mediatype .'" data-jw-mediaurl="'. $vidid .'" data-jw-loop="'. $NV_loop .'" data-jw-icons="'. $icons .'" data-jw-skin="'. $skin .'" data-jw-swfsrc="'. of_get_option('jwplayer_swf') .'" data-jw-autoplay="'. $NV_videoautoplay .'"></div></div>';
