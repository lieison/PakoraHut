<?php
	...
	class WPBakeryShortCode_icon extends WPBakeryShortCode {
		...
		public function content( $atts, $content = null ) {
			...
			return '<span class="fonticon '. $css_class .' '. ( !empty( $iconsize ) ? $iconsize .' ' : '' ) . ( !empty( $align ) ? $align .' ' : '' ) . ( !empty( $background ) ? 'background' : '' ) .'" '. ( !empty( $backgroundcolor ) ? 'style="background-color:'. $backgroundcolor .'"' : '' ) .'><i class="fa '. $name .'" '. ( !empty( $iconcolor ) ? 'style="color:'. $iconcolor .'"' : '' ) .'></i></span>';
