<?php
	...
	class WPBakeryShortCode_Plan extends WPBakeryShortCode {
		...
		public function content( $atts, $content = null ) {
			...
			$output = '';
			$css_class = apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'nv-pricing-plan group '.$featured.' '. $el_class .' '. $width, $this->settings['base']);
			$output .= "\n\t\t\t" . '<div class="'.$css_class.'">';	
			...
			$output .= "\n\t\t\t\t" . '<div class="nv-pricing-title '. ( !empty( $icon ) ? 'icon' : '' ) .' '. $color .'"><h4>'.esc_attr($title).'</h4>';
			$output .= "\n\t\t\t\t\t" . '<div class="nv-pricing-cost"><span class="price-value"><span class="price-currency">'. esc_attr($currency) .'</span>'. esc_attr($price) .'<span class="price-per">'. esc_attr($per) .'</span></span></div>';
			$output .= "\n\t\t\t\t" . '</div>';
